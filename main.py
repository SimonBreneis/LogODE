...
partition = np.linspace(0, 2, 5)
solution = log_ode(x=lambda t: np.array([t]), f_vec=[lambda y, dx: np.array([y*dx])], y_0=np.array([1.]), deg=1,
                   partition=partition)
plt.plot(np.linspace(0, 2, 1000), np.exp(np.linspace(0, 2, 1000)))
...
y_0 = np.array([0., 0.])
deg = 1
solution = log_ode(x, f_vec, y_0, deg, partition)
...
def df(y, dx):
    return (dx[0, 0] * np.array([logistic(y[1]) + y[0] - y[1], logistic(y[1]) ** 2 * (1 - logistic(y[1]))])
            + dx[0, 1] * np.array(
                [y[1] + logistic(y[0] - 2 * y[1]), logistic(y[1]) * (1 - logistic(y[1])) * logistic(y[0] - 2 * y[1])])
...
solution = log_ode(x, f_vec, y_0, deg, partition, n_steps=20, norm=l2, p=1)
...
f_vec = [
    lambda y, dx: np.einsum('ij,j', np.array([[y[1] - y[0], -y[1]], [logistic(y[1]), logistic(y[0] - 2 * y[1])]]), dx)]
y_0 = np.array([0., 0.])
partition, solution = log_ode_user(x, f_vec, y_0, 1., norm=l1, p=1)
plt.plot(solution[:, 0], solution[:, 1])
plt.show()
