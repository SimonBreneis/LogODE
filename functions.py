...
def discuss_example(x, f, y_0, N_min=1, N_max=3, T=1., n=16, g=None, g_grad=None, atol=1e-04, rtol=1e-02, method='RK45',
    ...
    def small_discussion():
        ...
        print(f'The final point of the solution given by the algorithm is {y[-1, :]}.')
    ...
    partition, y, prop_loc_err, N = lo.solve_fully_adaptive_error_representation(x=x, f=f, y_0=y_0, N_min=N_min,
                                                                                 ...
                                                                                 speed=speed)
    print(f'Finished solving the RDE using the fully adaptive error representation. It took {time.perf_counter() - tic} '
          f'seconds.')
    discuss()
    ...
    print(f'Finished solving the RDE using the fully adaptive error representation and the algorithm which tries '
          f'everything. It took {time.perf_counter() - tic} seconds.')
    discuss()

    print('Starting to solve the RDE without the error representation formula, where we solve only for the first level')
    tic = time.perf_counter()
    solver = lambda partition: lo.solve_fixed(x=x, f=f, y_0=y_0, N=N_min, partition=partition, atol=atol, rtol=rtol,
                                              method=method, compute_bound=False)
    y = lo.solve_error_tolerance(solver=solver, n=n, T=T, atol=atol, rtol=rtol, full_tensor_tolerance=False)
    ...
    solver = lambda partition: lo.solve_fixed_full(x=x, f=f, y_0=y_0, N=N_min, partition=partition, atol=atol,
                                                   rtol=rtol, method=method, compute_bound=False, N_sol=N_min)
    y = lo.solve_error_tolerance(solver=solver, n=n, T=T, atol=atol, rtol=rtol, full_tensor_tolerance=False)
    print(f'Finished solving the RDE without the error representation formula, where we solve only for the first level.'
          f' It took {time.perf_counter() - tic} seconds.')
    small_discussion()
