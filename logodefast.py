...
def solve_step_logsig(g, f, y_0, atol=1e-07, rtol=1e-04, method='RK45', compute_bound=False):
    ...
    def apply(flattened=True):
        ...
        if not compute_bound or f.norm is None:
            return lambda y: np.sum(np.array([f.derivative(y, i) @ (g.flatten(i) if flattened else g[i], i)
                                              for i in range(1, deg + 1)]), axis=0)

        def compute_vf_and_norm(y):
            while deg > len(f.local_norm):
                f.local_norm.append(0.)
                f.global_norm.append(0.)
            ls_norms = np.array([g.norm(i, f.norm) for i in range(1, deg + 1)])
            summands = np.array([f.derivative(y, i) @ (g.flatten(i) if flattened else g[i], i)
                                 for i in range(1, deg + 1)])
            vf = np.sum(summands, axis=0)
            for i in range(deg):
                local_local_norm = (f.norm(summands[i]) / ls_norms[i]) ** (1. / (i + 1))
                if local_local_norm > f.local_norm[i]:
                    f.local_norm[i] = local_local_norm
                    if local_local_norm > f.global_norm[i]:
                        f.global_norm[i] = local_local_norm
            return vf

        return compute_vf_and_norm

    return integrate.solve_ivp(lambda t, z: apply()(z), (0, 1), y_0, method=method, atol=atol,
                               rtol=rtol).y[:, -1]
